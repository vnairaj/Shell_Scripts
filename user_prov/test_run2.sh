...
EOF
						 ...
						 val_usrcrt=`grep "ORA-"  $SHELLDIR/create_deftbsp_defrole_$usrnm.log`
								...
								if [ -z "${val_usrcrt}" ];then
									...
									echo " Attention!! USER CREATION IS NOT SUCCESSFUL, check the inputs provided"
...
EOF
		...
		echo "**Grants Successful**"
                ...
                printf "Roles to be Granted, EX: ROLE1,ROLE2 (If none - Press Enter): "
                read grt_role
                if [ -z "${grt_role}" ]; ################# Default role condition with tablespace
                then
        sqlplus -s "sys/${pass}@${dbtns} as sysdba" <<EOF >/dev/null 2>&1
spool $SHELLDIR/create_custtbsp_defrole_$usrnm.log
CREATE USER ${usrnm} IDENTIFIED BY "Temp#123" DEFAULT TABLESPACE  ${tbsp}  PASSWORD EXPIRE ACCOUNT UNLOCK;
grant connect to ${usrnm};
spool off
EOF
										 val_usrcrt=`grep "ORA-"  $SHELLDIR/create_custtbsp_defrole_$usrnm.log`
								...
								if [ -z "${val_usrcrt}" ];then
									...
									echo " Attention!! USER CREATION IS NOT SUCCESSFUL, check the inputs provided"
									echo "If INPUTS are correct, send the below screenshot to DB team"
									cat $SHELLDIR/create_custtbsp_defrole_$usrnm.log
							fi
						

   else    ########If role exists and Custom tablespace condition
                sqlplus -s "sys/${pass}@${dbtns} as sysdba" <<EOF >/dev/null 2>&1
spool $SHELLDIR/create_custbsp_custrole$usrnm.log
CREATE USER ${usrnm} IDENTIFIED BY "Temp#123" DEFAULT TABLESPACE ${tbsp} PASSWORD EXPIRE ACCOUNT UNLOCK;
grant connect,${grt_role}  to ${usrnm};
spool off
EOF

										 val_usrcrt=`grep "ORA-"  $SHELLDIR/create_custbsp_custrole$usrnm.log`
								...
								if [ -z "${val_usrcrt}" ];then
									...
									echo " Attention!! USER CREATION IS NOT SUCCESSFUL, check the inputs provided"
									echo "If INPUTS are correct, send the below screenshot to DB team"
									cat $SHELLDIR/create_custbsp_custrole$usrnm.log
							fi
